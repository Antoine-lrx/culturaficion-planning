...
export async function onRequestPost({ request, env }) {
  ...
  const { id, type, title, monthKey, date, lieu, helloassoSlug, status, proposedBy, notes } = body || {};
  ...
  await env.DB.prepare(
    `INSERT INTO events (id, type, title, month_key, date, lieu, helloasso_slug, status, proposed_by, voters, notes, registered, revenue, expenses, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, '[]', ?, NULL, NULL, NULL, ?)`
  ).bind(id, type, title, monthKey, date || null, lieu || null, helloassoSlug || null, status, proposedBy || null, notes || null, createdAt).run();
